...
def test_account_creation(session):
    ...
    assert retrieved_account.profile_icon_id == 123
...
def test_match_creation(session):
    ...
    assert retrieved_match.assists == 5
    assert retrieved_match.champ_level == 10
    assert retrieved_match.champion_id == 123
    assert retrieved_match.champion_name == "TestChampion"
    assert retrieved_match.deaths == 2
    assert retrieved_match.gold_earned == 1500
    assert retrieved_match.individual_position == "Top"
    assert retrieved_match.item_0 == 1001
    assert retrieved_match.item_1 == 1002
    assert retrieved_match.item_2 == 1003
    assert retrieved_match.item_3 == 1004
    assert retrieved_match.item_4 == 1005
    assert retrieved_match.item_5 == 1006
    assert retrieved_match.item_6 == 0
    assert retrieved_match.kills == 8
    assert retrieved_match.lane == "Top"
    assert retrieved_match.perk_0 == 8000
    assert retrieved_match.perk_1 == 8100
    assert retrieved_match.puuid == "unique-puuid"
    assert retrieved_match.riot_id_game_name == "TestGameName"
    assert retrieved_match.riot_id_tag_line == "TestTagline"
    assert retrieved_match.summoner_1_id == 1
    assert retrieved_match.summoner_2_id == 2
    assert retrieved_match.summoner_id == "unique-summonerId"
    assert retrieved_match.team_id == 100
    assert retrieved_match.team_position == "Top"
    assert retrieved_match.total_damage_dealt_to_champions == 20000
...
def test_participant_creation(session):
    ...
    assert retrieved_participant.champion_id == 123
    assert retrieved_participant.champion_name == "TestChampion"
    assert retrieved_participant.riot_id_game_name == "TestGameName"
    assert retrieved_participant.riot_id_tagline == "TestTagline"
    assert retrieved_participant.team_id == 400
    assert retrieved_participant.team_position == "Jungler"
